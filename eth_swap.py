...
for pb, pk in zip(wallet_addresses.wallet_address_ls, wallet_addresses.pk_ls):
    ...
    output_abi_dir = file_ls[r]
    output_symbol_name = output_abi_dir.split('_')[0]

    with open(target_chain_dir + '/' + 'eth_abi.json') as abi_file:
        eth_contract_abi = json.load(abi_file)

    eth_token_contract = w3.eth.contract(address= Web3.to_checksum_address(''), abi=eth_contract_abi)
    eth_token_balance = eth_token_contract.functions.balanceOf(user_addr).call()
    eth_token_balance *= 0.8
    ...
    quote_request_body = {
        ...
        "inputTokens": [
            {
                "tokenAddress": eth_token_contract,
                "amount": str(eth_token_balance),
            }
        ],
        "outputTokens": [
            {
